...
package sunag.sea3d
{
	...
	{		
		...
		public function SEAMesh(name:String, data:ByteArray, sea:SEA3D)
		{
			...
			if (instance.length == 0)
			{
				...
				{
					subMesh[i] = new SubMesh(ByteArrayUtils.readUTFTiny(data), data.readUnsignedShort());					
				}
				...
				if (data.readBoolean())					
					normal = new Vector.<Number>(vertex.length);									
				...
				if (normal)
				{
					for (i=0;i<vertex.length;i+=3) 
					{
						...
						normal[i+1] = data.readFloat();
						normal[i+2] = data.readFloat();
					}
				}				
			} 
			else 
