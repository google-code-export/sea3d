...
package sunag.sea3d.away3d
{
	...
	public class Away3D extends Bridge
	{
		...
		private function readCamera(name:String, fov:Number, position:Vector3D, rotation:Vector3D, target:Vector3D, animation:String):void
		{
			...
			if (animation) 
			{
				addAnimation(new CameraAnimation(cam, getAnimation(animation).cloneData(), position, rotation, target, fov), animation);
			}
		}
		...
		private function readLight(light:LightBase, name:String, color:int, multiplier:Number, position:Vector3D, rotation:Vector3D, target:Vector3D, shadow:Boolean, animation:String):void
		{
			...
			if (animation) 
			{
				addAnimation(new LightAnimation(light, getAnimation(animation).cloneData(), position, null, target, color, multiplier), animation);
			}
		...
		public function readHelper(sea:SEAHelper):void
		{
			...
			if (sea.animation)
			{
				addAnimation(new HelperAnimation(helper, getAnimation(sea.animation).cloneData(), sea.position, sea.rotation, sea.scale, sea.color), sea.animation);
			}
