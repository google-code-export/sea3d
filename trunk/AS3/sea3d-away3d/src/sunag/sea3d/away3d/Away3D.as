...
package sunag.sea3d.away3d
{
	...
	import sunag.sea3d.away3d.animation.CameraAnimation;
	import sunag.sea3d.away3d.animation.HelperAnimation;
	import sunag.sea3d.away3d.animation.LightAnimation;
	...
	public class Away3D extends Bridge
	{
		...
		public function readMaterial(sea:SEAMaterial):void
		{
			...
			if (sea.selfIllumColor > 0)
			{
				...
				mat.ambientColor = sea.selfIllumColor;
				mat.ambient = 1;
			}
			...
			if (mat)
			{
				...
				if (tex && tex.layer.length > 0)
				{
					...
					{		
						...
						addAnimation(textureAnimation, name);
					}
				}
			}
		}
		...
		public function readMesh(sea:SEAMesh):void
		{
			...
			if (sea.instance.length == 0)
			{
				geo = new Geometry();	
				
				for(i=0;i<sea.subMesh.length;i++)
				{
					geo.addSubGeometry(buildSubMesh(sea.subMesh[i], sea.vertex, sea.uv));
				}
				
				mesh = new Mesh(geo, getColorMaterial(sea.color));
			}
			else 
			{
				// Detect Animation						
				...
				mesh = new Mesh(instMesh.geometry, getColorMaterial(sea.color));
			...
			mesh.rotationX = sea.rotation.x;
			mesh.rotationY = sea.rotation.y;
			mesh.rotationZ = sea.rotation.z;
			
			mesh.scaleX = sea.scale.x;
			mesh.scaleY = sea.scale.y;
			mesh.scaleZ = sea.scale.z;	
			...
			if (sea.animation) 
			{
				addAnimation(new MeshAnimation(mesh, getAnimation(sea.animation).cloneData(), sea.position, sea.rotation, sea.scale), sea.name);
			}
		}				
		
		public function buildSubMesh(subMesh:sunag.sea3d.mesh.SubMesh, vertex:Vector.<Number>, uvData:Vector.<UVData>):SubGeometry
		{
		...
		private function readCamera(name:String, fov:Number, position:Vector3D, rotation:Vector3D, target:Vector3D, animation:String):void
		{
			...
			if (animation) 
			{
				addAnimation(new CameraAnimation(cam, getAnimation(animation).cloneData(), position, rotation, target, fov), name);
			}
		}
		...
		public function readLightTarget(sea:SEALightTarget):void
		{
			readLight
			(
		...
		public function readLightFree(sea:SEALightFree):void
		{
			readLight
			(
				new DirectionalLight(), 
		...
		private function readLight(light:LightBase, name:String, color:int, multiplier:Number, position:Vector3D, rotation:Vector3D, target:Vector3D, shadow:Boolean, animation:String):void
		{
			light.color = color;
			...
			else if (target) light.lookAt(target);
			...
			if (animation) 
			{
				addAnimation(new LightAnimation(light, getAnimation(animation).cloneData(), position, null, target, color, multiplier), name);
			}
		...
		public function readHelper(sea:SEAHelper):void
		{
			...
			if (sea.animation)
			{
				addAnimation(new HelperAnimation(helper, getAnimation(sea.animation).cloneData(), sea.position, sea.rotation, sea.scale, sea.color), sea.name);
			}
