package away3d.containers
{
	...
	public class View3D extends Sprite
	{
		...
		private var _sunagMenu:ContextMenuItem;
		...
		private function visitSunagWebsite(e:ContextMenuEvent):void 
		{
			var url:String = Away3D.WEBSITE_URL;
			var request:URLRequest = new URLRequest("http://www.sunag.com.br/");
			try {
				navigateToURL(request);
			} catch (error:Error) {
		...
		private function initRightClickMenu():void
		{
			...
			_sunagMenu = new ContextMenuItem("Sunag Entertainment", true, true, true);
			...
			_sunagMenu.addEventListener(ContextMenuEvent.MENU_ITEM_SELECT, visitSunagWebsite);
		...
		private function updateRightClickMenu():void
		{
			if (_rightClickMenuEnabled)
				_ViewContextMenu.customItems = _sourceURL? [_menu0, _sunagMenu, _menu1] : [_menu0, _sunagMenu];
			else
