package away3d.audio
{
	...
	*/
	public class AdvSound3D extends ObjectContainer3D
	{
		...
		*/
		public function AdvSound3D(sound:Sound, reference:ObjectContainer3D = null, driver : ISound3DDriver = null, volume:Number = 1, scale:Number = 1000)
		{			
			_sound = sound;
			
			_driver = driver || new SimplePanVolumeDriver();
			_driver.sourceSound = _sound;
			_driver.volume = _volume = volume;
			_driver.scale = scale;
		...
		public function set reference(val : ObjectContainer3D) : void
		{
			if (_reference == val)
				return;
			...
			if (_reference)
			{					
				_reference.removeEventListener(Object3DEvent.SCENETRANSFORM_CHANGED, onSceneTransformChanged);
			}
		...
		*/
		...
		private function update():void
		{
			...
			_driver.volume = _reference ? _volume : 0;
			if (_soundMixer3D) _driver.volume *= _soundMixer3D.volume;
		...
		{
			if (_repeat && _playing) _driver.play();  
			else dispatchEvent(ev.clone());
		}
