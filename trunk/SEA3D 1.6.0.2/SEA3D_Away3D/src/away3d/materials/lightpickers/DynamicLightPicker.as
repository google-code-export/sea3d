package away3d.materials.lightpickers
{
	...
	{			
		...
		public var bestLights : Boolean = true;
		...
		public function update(lights:Array=null):void
		{
			...
			{			
				...
				directionalLightInstance.transform = shadow.sceneTransform;
		...
		override public function collectLights(renderable : IRenderable, entityCollector : EntityCollector) : void
		{
			...
			else
			{
				for each(light in _lights)
				{
					...
					if (dist < nearest)
					{
						...
						else if (light is DirectionalLight) pLights.unshift( light ); 
					...
					else
					{
						...
						else if (light is DirectionalLight) pLights.push( light ); 
					}								
				}
			}
			...
			for(; i < _directionalLightLimit; i++)
			{
				...
				if (i < dLights.length)
				{
					...
					dirLight.transform = dirLightTarget.sceneTransform;
