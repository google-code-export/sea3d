package away3d.animators
{
	...
	public class SkeletonAnimator extends AnimatorBase implements IAnimator
	{
		...
		arcane var numUsedVertexConstants:uint;
		
		override public function clone():IAnimator
		{
			return new SkeletonAnimator(SkeletonAnimationSet(_animationSet), _skeleton, _forceCPU);
		}
		...
		/**
		 * returns the current local skeleton pose output from the animator.
		 * 
		 * @see away3d.animators.data.SkeletonPose
		 */
		public function get skeletonPose():SkeletonPose
		{
        ...
        public function testGPUCompatibility(pass : MaterialPassBase) : void
        {
			if (!_useCondensedIndices && (_forceCPU || _jointsPerVertex > 4 || pass.numUsedVertexConstants + _numJoints * 3 > 128)) {
				numUsedVertexConstants = pass.numUsedVertexConstants;
		...
		override protected function updateDeltaTime(dt : Number) : void
		{
			...
			_globalMatricesDirty = _globalPoseDirty = true;
		...
		public function updateGlobalPose() : void
		{
			...
			//get global pose
			localToGlobalPose(skeletonPose, _globalPose, _skeleton);
		}
