package away3d.core.managers
{
	...
	public class Mouse3DManager
	{
		...
		private var _mouseView3D : View3D;
		private var _activeView : Boolean = false;
		 ...
		 */
		public function Mouse3DManager(view3D:View3D)
		{
			_view3D = view3D;
		}
		...
		public function updateCollider(view : View3D) : void
		{
			...
			if (_activeView && (_forceMouseMove || _updateDirty)) { // If forceMouseMove is off, and no 2D mouse events dirtied the update, don't update either.
				_collidingObject = _mousePicker.getViewCollision(_view3D.mouseX, _view3D.mouseY, _view3D);
			}
		...
		{
			if (_collidingObject && _mouseDown.object == _collidingObject.entity) 
			{
		...
		{
			if (_mouseView3D && _mouseView3D != view)
				disableMouseListeners(_mouseView3D);
