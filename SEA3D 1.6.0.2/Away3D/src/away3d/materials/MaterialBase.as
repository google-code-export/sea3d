...
//
		...
		/**
		 * Render with skin or not
		 * @author Sunag Entertainment
		 * */
		arcane function set animationSet(val:IAnimationSet):void
		{
			if (_animationSet == val) return;
			_animationSet = val;
			for (var i : int = 0; i < _numPasses; ++i)
				_passes[i].animationSet = _animationSet;
			_depthPass.animationSet = _animationSet;
		...
		arcane function get animationSet():IAnimationSet
		{
